"""Оркестрация коммерческого поиска поставщиков после подтверждения изделия."""
...
async def run_search(*, request_id: int, product: str, requirements: list[str]) -> SearchSummary:
    ...
    registry = await registry_service.check_product(product, request_id=request_id, cache=True)
    best = registry.best
    ...
    for item in search.suppliers:
        ...
        if screening.suspicious:
            tainted.add(item.site or item.name)
        inputs.append(SupplierInput(
            name=item.name,
    ...
    async with session_scope() as session:
        key_to_id = await repo.upsert_suppliers(session, inputs)

    to_scrape = [x for x in search.suppliers if x.site][:MAX_SITES_TO_SCRAPE]
    scrape_task = (
        get_firecrawl_service().scrape_many(
            [x.site for x in to_scrape], product, request_id=request_id
        ) if to_scrape else _nothing()
    )
    ...
    scrapes = {r.url: r for r in results}
    ...
    for item in search.suppliers:
        ...
        ru_match, basis = _site_registry_match(product, scrape, best)
        allowed, gate_basis = _supplier_gate(item, product, scrape, best, ru_match)
        if not allowed:
            gated += 1
            logger.info("Supplier Gate: исключён %s — %s", item.site or item.name, gate_basis, extra=extra)
            continue

        accepted.append(item)
        evidence_url = (
            scrape.evidence_url if scrape and getattr(scrape, "evidence_url", None) else item.site
        ) or None
        flags = _unrega_flags(unrega, ru_site_match=ru_match, ru_match_basis=basis)
        flags["supplier_role"] = getattr(item, "role", "candidate") or "candidate"
        flags["role_evidence_url"] = getattr(item, "role_evidence_url", "") or ""
        flags["role_evidence"] = getattr(item, "role_evidence", "") or ""

        candidates.append(CandidateInput(
            supplier_id=sid,
            site_claims=scrape.claims_stock if scrape and scrape.ok else None,
            site_url=evidence_url,
            ...
            raw={
                ...
                "registry_match": {"site_matches_ru": ru_match, "basis": basis},
                "supplier_gate": {"allowed": True, "basis": gate_basis},
                "search": {
                    "note": item.note,
                    "source": item.source_url,
                    "role": getattr(item, "role", "candidate"),
                    "role_evidence_url": getattr(item, "role_evidence_url", ""),
                    "role_evidence": getattr(item, "role_evidence", ""),
                },
                "scrape": {
                    ...
                    "injection_suspected": bool(
                        ...
                        or (item.site or item.name) in tainted
                    ),
                },
            },
        ))
    ...
    async with session_scope() as session:
        ...
        await _enrich_contacts(session, accepted, scrapes, key_to_id)
    ...
    summary.budget_exceeded = budget.exceeded(request_id)
    logger.info(
        "Подбор: discovery %s, проверено %s, Supplier Gate исключил %s, в отчёт %s",
        summary.total_found, summary.scraped, gated, len(candidates), extra=extra,
    )
    return summary
...
def _normalise_ru(v: str) -> str:
    return re.sub(r"[^a-zа-яё0-9]", "", (v or "").lower())


def _distinctive_terms(text: str) -> set[str]:
    return {
        w for w in re.findall(r"[A-Za-zА-Яа-яЁё0-9-]{4,}", (text or "").lower())
        if w not in _GENERIC_MATCH_TERMS and not w.isdigit()
    }
...
def _holder_matches(item: object, best: Any) -> bool:
    if best is None:
        return False
    holder = str(getattr(best, "holder", "") or "").lower()
    name = str(getattr(item, "name", "") or "").lower()
    domain = str(getattr(item, "site", "") or "").lower()
    terms = [
        x for x in re.findall(r"[a-zа-яё0-9]{5,}", holder)
        if x not in {"общество", "ограниченной", "ответственностью"}
    ]
...
def _supplier_gate(
    ...
    if _holder_matches(item, best):
        return True, "держатель/производитель РУ"
    if scrape is None or not scrape.ok or not scrape.markdown:
        return False, "страница не проверена"
    text = scrape.markdown.lower()
    commercial = bool(
        getattr(scrape, "claims_stock", None) is True
        or getattr(scrape, "price", None) is not None
        or any(m in text for m in _SUPPLIER_MARKERS)
    )
    strong_commerce = bool(
        getattr(scrape, "claims_stock", None) is True
        or getattr(scrape, "price", None) is not None
        or any(m in text for m in _STRONG_COMMERCE_MARKERS)
    )
    negative = sum(1 for m in _NON_SUPPLIER_MARKERS if m in text)
    product_hit = any(t in text for t in _distinctive_terms(product))
    if negative and not strong_commerce:
        return False, "реестр/аналитика/сертификационный ресурс, а не продавец товара"
    if product_hit and commercial:
        return True, "товар + коммерческие признаки на сайте"
...
def _site_registry_match(product: str, scrape: ScrapeResult | None, best: Any) -> tuple[bool | None, str]:
    # Сохраняется только как диагностическая совместимость со старыми данными.
    # На коммерческое ранжирование это больше не влияет.
    ...
    page = scrape.markdown.lower()
    expected = _normalise_ru(str(best.ru_number))
    compact = _normalise_ru(page)
    if expected and expected in compact:
        return True, "точный номер РУ найден на странице поставщика"
    nums = {_normalise_ru(m.group(0)) for m in PAGE_RU_RE.finditer(scrape.markdown)}
    nums.discard("")
    ...
    registry_text = " ".join(str(v or "") for v in (
        getattr(best, "product_name", None), getattr(best, "holder", None), raw_text
    )).lower()
    strong = {t for t in _distinctive_terms(product) if t in registry_text}
    matched = {t for t in strong if t in page}
    if matched:
        return None, "совпал бренд/модель, но номер РУ на странице не найден"
    return None, "на странице недостаточно данных для привязки к РУ"
...
async def _enrich_contacts(
    ...
    for item in suppliers:
        ...
        if scrape and scrape.ok and (scrape.email or scrape.phone):
            updates.append(SupplierInput(
...
async def build_report(
    ...
    *,
    ...
    views = [CandidateView(
        ...
        supplier_id=int(r.supplier_id or 0),
        supplier_name=str(r.supplier_name),
        domain=r.domain,
        email=r.email,
        phone=r.phone,
        site_claims=r.site_claims,
        site_url=r.site_url,
        site_price=r.site_price if isinstance(r.site_price, Decimal) else None,
        ru_number=r.ru_number,
        ru_holder=r.ru_holder,
        ru_valid=r.ru_valid,
        ru_registry=r.ru_registry,
        registry_state=_registry_state(r),
        ru_site_match=_ru_site_match(r.unrega_flags),
        ru_match_basis=_ru_match_basis(r.unrega_flags),
        supplier_role=_supplier_role(r.unrega_flags),
        role_evidence_url=_role_evidence_url(r.unrega_flags),
        role_evidence=_role_evidence(r.unrega_flags),
        unrega_flags=_unrega_items(r.unrega_flags),
    ...
    await repo.set_candidate_ranks(session, {
        v.candidate_id: i for i, v in enumerate(ordered, 1)
    })
...
def _registry_state(r: object) -> str:
    ...
    if s in (RegistryState.FOUND, RegistryState.NOT_FOUND, RegistryState.UNAVAILABLE):
        return str(s)
    return RegistryState.FOUND if getattr(r, "ru_number", None) else RegistryState.UNAVAILABLE


def _unrega_items(v: object) -> list[str]:
    return [str(x) for x in v.get("items", []) or []] if isinstance(v, dict) else []


def _ru_site_match(v: object) -> bool | None:
    m = v.get("ru_site_match") if isinstance(v, dict) else None
    return m if isinstance(m, bool) else None


def _ru_match_basis(v: object) -> str | None:
    b = v.get("ru_match_basis") if isinstance(v, dict) else None
    return str(b) if b else None


def _supplier_role(v: object) -> str:
    role = v.get("supplier_role") if isinstance(v, dict) else None
    return str(role) if role in {"manufacturer", "official_distributor", "seller", "candidate"} else "candidate"


def _role_evidence_url(v: object) -> str | None:
    value = v.get("role_evidence_url") if isinstance(v, dict) else None
    return str(value) if value else None


def _role_evidence(v: object) -> str | None:
    value = v.get("role_evidence") if isinstance(v, dict) else None
    return str(value) if value else None
...
def _unrega_state(rows: Sequence[Any]) -> str:
    for r in rows:
        ...
        if isinstance(f, dict) and f.get("state") in (
            RegistryState.FOUND, RegistryState.NOT_FOUND, RegistryState.UNAVAILABLE
        ):
            return str(f["state"])
