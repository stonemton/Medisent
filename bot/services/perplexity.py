...
# Домены, которые не являются российскими поставщиками товара: агрегаторы,
# маркетплейсы, справочники, соцсети и страницы закупок. Источник информации
# может быть полезен для поиска, но кандидатом в поставщики становиться не должен.
...
или продавцов, у которых можно запросить/купить указанное изделие. Не включай
...
class PerplexityService:
    ...
    async def find_suppliers(self, product: str, *, requirements: list[str] | None = None, request_id: int | None = None, model: str = DEFAULT_MODEL) -> SearchOutcome:
        ...
        query = f"Российские поставщики и дистрибьюторы медицинского изделия: {product}.{extras} Нужны названия компаний, их официальные сайты и контакты."
        agent_model = f"perplexity/{model}" if model and "/" not in model else model
        result = await self._client.post(
            API_URL, operation="search", request_id=request_id, cost_usd=pricing.flat_cost("perplexity"),
            json={"model": agent_model or DEFAULT_MODEL, "instructions": self._instruction() + "\n" + SUPPLIERS_SCHEMA_HINT, "input": query,
                  "tools": [{"type": "web_search", "filters": {"search_recency_filter": "year"}}]},
        )
        if not result.ok:
        ...
        logger.info("Perplexity: по «%s» кандидатов %s, источников %s", product, len(suppliers), len(citations), extra=log_extra(request_id))
...
def _parse_suppliers(content: str, citations: list[str]) -> list[FoundSupplier]:
    ...
    for row in rows:
        ...
        if site and not is_supplier_domain(site): continue
    ...
    # Добираем только источники с допустимыми доменами. Известные агрегаторы и
    # маркетплейсы выше отсекаются независимо от того, назвала ли их модель.
    for url in citations:
        if not is_supplier_domain(url): continue
        key = domain_of(url)
        if key in seen: continue
        seen.add(key)
