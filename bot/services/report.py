...
РУ на изделие и связь конкретного товара поставщика с этим РУ — разные факты.
Первое берётся из официального реестра; второе подтверждается только данными
со страницы конкретного поставщика. Отсутствие подтверждения не превращается
в утверждение, что поставщик работает именно по найденному РУ.
"""
...
def _ru_match_line(view: CandidateView) -> str | None:
    """Отдельно показывает, связан ли товар на сайте поставщика с найденным РУ."""
    ...
    if view.ru_site_match is True:
        return f"связь товара поставщика с РУ подтверждена{f' · {basis}' if basis else ''}"
    if view.ru_site_match is False:
        return f"⚠️ на странице поставщика найдено противоречие с РУ{f' · {basis}' if basis else ''}"
    return f"связь товара поставщика с этим РУ не подтверждена{f' · {basis}' if basis else ''}"
        ...
        "candidates": [
            {
                ...
                "registry": {
                    ...
                    "site_match": c.ru_site_match,
                    "match_basis": c.ru_match_basis,
                },
...
def render(report: Report) -> list[str]:
    ...
    for index, view in enumerate(report.candidates, start=1):
        ...
        if ru_match:
            lines.append(f"   Привязка РУ: {ru_match}")
